...
namespace LmUtils
{
    public class StringUtilities
    {
        ...
        {
            string[] dims = new string[]{"КБ", "МБ", "ГБ", "ТБ"};
        ...
        public static Collection<PathClassIdentity> ParseClassPath(string path)
        {
            ...
            foreach (string path_part in path_parts)
            {
                ...
                Match match = Regex.Match(path_part, @"[^\w]*(?'name'[\w]+)\[(?'index'[\d]+)\].*", RegexOptions.IgnorePatternWhitespace);
        ...
        public static string Encode(string str, string rgbKey, string rgbIV)
        {
            ...
            ICryptoTransform encoder = crypto_provider.CreateEncryptor(Encoding.Default.GetBytes(rgbKey), Encoding.Default.GetBytes(rgbIV));
        ...
        {
            TripleDESCryptoServiceProvider crypto_provider = new TripleDESCryptoServiceProvider();
            ICryptoTransform decoder = crypto_provider.CreateDecryptor(Encoding.Default.GetBytes(rgbKey), Encoding.Default.GetBytes(rgbIV));
